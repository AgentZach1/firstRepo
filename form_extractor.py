...
def get_all_forms(url):
     """Returns all tags found on a webpage's url"""
     # GET request
...
def get_form_details(form):
     ...
     # if not specificed, get the default in HTML
...
if __name__ == "__main__":
    ...
    # iteratte over forms
     for i, form in enumerate(forms, start=1):
          ...
          #pprint(form_details, width=1)
          for detail in form_details:
               print("{}".format(detail))
               if (detail == "inputs"):
                    for attr in form_details[detail]:
                         if attr['type'] != 'hidden':
                              print(attr)
               else:
                    print(form_details[detail])
          #for detail in form_details:
          #     print(detail)
